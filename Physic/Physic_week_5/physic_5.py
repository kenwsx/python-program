...
output_detail = open("elect_detail.txt", "w")  # 印出內部積分的特定半徑情形
    ...
    for rp in arange(0, charge.radius, drp):          # 積分開始，半徑從 0 到 charge.radius
        for thetap in arange(0, math.pi, dthetap):         # 天頂角從 0 到 pi（北極到南極）
            for phip in arange(0, 2*math.pi, dphip):       # 方位角從 0 到 2*pi（繞一整圈）
                ...
                # 我發現若用第二種方法，在距離很小的時候會有數值暴衝，不過用第三種可以避免
                # 第三種會有兩個值暴衝，當積分距離與半徑距離皆為在r=3.0與r=4.5的狀況
                # 第四點為這兩種暴衝的積分細節
                # 即使把全部狀況用上，圖形依舊會有突起處，標準差巨大
                ...
                # 4.在 3.的時候，若有以下兩種情形，值會超大
                '''if rp == 3.0 and thetap == 1.5707963267948966 and phip == 0.6283185307179586:
                    continue
                if rp == 4.5 and thetap == 1.5707963267948966 and phip == 0.9424777960769379:
                    continue'''
                ...
                # 可藉由調整if中的條件，紀錄特定電場/電位勢距離的積分全部狀況
                # 可用關鍵字"e"尋找數值超大的條件
                # 此處用中文表示原因為電場的e會與10次方的e字母一樣
                '''if rf == 3.0000000001 or rf == 4.5000000001:
                    print(f"{rp},{thetap},{phip}\n電位勢:{dV}\n電場:{dE.mag}\n",
                          file=output_detail)'''
