...
for multiple in range(1, 21):
    ...
    for theta in arange(0, 180, 180/10):   # 天頂角根據輸入步數，決定總步

        theta = theta*math.pi/180  # 換成弧度

        for phi in arange(0, 360, 360/10):  # 方位角根據輸入步數，決定總步

            if theta != 0 or phi != 0:
                continue
            phi = phi*math.pi/180  # 換成弧度

            r = 200 * vector(     # 算出要計算電場的位置（距離球心有多少距離）
            ...
            dthetap = math.pi/(6*multiple)
            # 將方位角 phip 分成許多小步計算
            dphip = math.pi*2/(3*multiple)
            ...
            V_r.plot(multiple, E.mag)  # 在V_r 曲線圖上畫一個點
            # 如果是第一個方位，就存入總電場的值
    ...
    # 把總電場的值放入字典
    E_dic["multiple_"+str(multiple)] = electric_field

# 印出全部半徑電場的值（在theta = 0, phi = 0 的方位）
for i in range(1, 21):
    result = E_dic["multiple_"+str(i)]
    print(f"multiple {i}:{result}", file=output)

# 印出全部比較半徑電場的值
for i in range(2, 21):
    elect_bigger = E_dic["multiple_"+str(i)]
    elect_smaller = E_dic["multiple_"+str(1)]
    result = elect_bigger / elect_smaller
    print(f"Multiple {i}/1:{result:.6e}", file=output, sep='')
